...
export interface AgentWorkspaceConfig {
  ...
  /** PROMPT 17: Linked tools */
  tools?: string[];
  /** PROMPT 17: Linked skills */
  skills?: string[];
 ...
 *
 * PROMPT 17: Now also writes tools/skills references to agent.json
 *
 * This creates:
 * 1. Agent directory in workspace
 * 2. agent.json config file (with tools/skills refs)
 * 3. system-prompt.md (with tool/skill info)
 *
 * This does NOT:
...
export async function materializeAgent(
  ...
  try {
    ...
    const workspaceConfig: AgentWorkspaceConfig = {
      ...
      capabilities,
      config: agentConfig,
      // PROMPT 17: Include tools and skills references
    ...
    const systemPrompt = generateBasicSystemPrompt(agentName, agentType, description, capabilities, tools, skills);
 ...
 * PROMPT 17: Now includes tools and skills info
 */
...
You can use these tools to accomplish tasks. Call tools when needed.
`;
  }
...
These skills provide additional capabilities you can leverage.
`;
  }
...
---
*Generated by OCAAS Agent Materialization (BLOQUE 9 + PROMPT 17)*
